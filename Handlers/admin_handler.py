...
class suggests_Handler(TornadoRequestBase):
    @authentication()
    def get(self):
        admin_id = self.session.get('id_admin')
        all_suggest = Suggest().select()
        ...
        self.render('admin/suggests.html', date=date, all_suggest=all_suggest, user=self.user)

class delsuggest_Handler(TornadoRequestBase):
    @authentication()
    def post(self, *args, **kwargs):
        suggest_id = self.get_argument('suggest_id')
        Suggest().select().where(Suggest.id == suggest_id).get().delete_instance()
        self.write("success")
